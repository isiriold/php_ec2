...
# dts changes start -->
function isLogin() {
//check if session has "member" set 
if(! (isset($_SESSION['member'] )) ) { return 0;}
//check if member has UserID 
if(! (isset($_SESSION['member']['userid'] )) ) { return 0;}
return 1;
}
function commitORdisplay()
{
   $a = getKey("commit", $GLOBALS["LOCALGET"]);
   if( strlen($a) == 0 ) { $a = getKey("commit", $GLOBALS["LOCALPOST"]); }
   if( strlen($a) == 0 ) { $a = "0"; }
   return $a;
}
...
return <<<zz
<a href="javascript:$func //" class="btnlnk ds clr2 $class" title="" id="$id" name="$id">$display</a>
zz;
} //makeButton
...
$type = "mp4";
$rtn = <<<zz
<img src="site/img/video_icon.png" class="adminMOVIEFrame imgWrap" />
<div onclick="javascript:plyVidWrkDiv('workerdiv','workerdivinner','workerdivcontent','?TASK=OPENWORKDIV&SRCVID=$content&TYPE=$type&RETURNTASK=ADMINPRODUCT','workerdivtitle');" class="movieLink imgWrap"></div>
zz;
break;
...
function makeProductDetailOpen( $idx = "",$band = "",$bandid = "",$type = "",$typeid = "", $typecode="",$title = "",$description = "",$content ="",$BandOptions=array(), $TypeOptions=array(), $TPoptions = "", $active=1, $sort=100,$productid, $curtask="",$formfix=false )
{
...
if( is_numeric( $idx) ) { $tmpFrm = "_$idx"; $tmpFrmClass = "brdr4 mrgn1 pad3"; } else { $tmpFrm = "";$tmpFrmClass = "";}
$theFormID = "deetForm$tmpFrm";
$theFormClass = $tmpFrmClass;
...
$tmpBtn = makeButton( $type="BUTTON", $display="Add", $id="", $func="newTag('$idx');", $class="" );
...
$mBand = makeInput( "select", "detailband_$idx", "txtsel", "$bandid", $BandOptions, $sjsFunc );
# dts -- **** start
$productDescription = makeInput("textarea","detaildescription_$idx","txt3 txtDescrp","$description", "", $tjsFunc);
# dts -- **** end
...
$mType = makeInput( "select", "detailtype_$idx", "txtsel", "$typeid", $TypeOptions, $typeFunc );
...
<div class="ht2">
	<div class="fl1 wid6"><label for="detailtitle_$idx">Title</label></div>
	<div class="fl1">$mTitle</div>
...
<br/>
<div class="ht6">
	<div class="fl1 wid6"><label for="detaildescription_$idx">Description</label></div>
	<div class="fl1">$productDescription</div>
	<div class="clear"></div>
</div>
<br/>
<div class="ht2">
	 <div class="fl1 wid6"><label for="detailband_$idx">Band</label></div>
	 <div class="fl1 pad1">$mBand</div>
	 <div class="clear"></div>
</div>
<br/>
<div class="ht2">
	 <div class="fl1 wid6"><label for="detailtype_$idx">Type</label></div>
...
<div class="ht2">
	 <div class="fl1 wid6"><label for="active_$idx">Active</label></div>
	 <div class="fl1 pad1">$mActive</div>
	 <div class="fl1 wid6"><label for="sort_$idx">Sort</label></div>
...
function makeProductDetailClose( $idx = "", $content ="",$formfix=false ) {
$ret = "";
if($formfix == true){
$ret =  <<<zzz
$content
...
$content
	 ...
	 <div class="clear"></div>
</div> <!-- non risi productdetail_$idx -->
<!--<hr class="hr1" />-->
...
$type = "mp4";
$rtn .= <<<zzz
<img onclick="javascript:plyVidWrkDiv('workerdiv','workerdivinner','workerdivcontent','?TASK=OPENWORKDIV&SRCVID=$Lbvalue&TYPE=$type&RETURNTASK=PRODUCT&PRODUCTID=1','workerdivtitle');" src="site/img/video_icon.png" class="adminMOVIEFrame1 imgWrap $type" />
...
/* --- this is with description ----
$rtn=<<<xx
<div class="outsetbox1 boxshadow fl1 padR1">
	 <div><b>$Lbname</b></div>
	 <hr class="bgAlt" />
	 $rtn
	 <div class="detailDescription">$Lbdescription</div>
</div>
xx;
*/
$rtn=<<<xx
...
switch( $MSG ) {
		...
		break;
		case "ORDERS":
		$MSG = "Your order is successfully made.";
		break;
		case "ASK":
		$MSG = "Your question is successfully submitted.";
		break;
		case "SIGNUP":
		$MSG = "Your account is successfully created, Please login with your login and password.";
		break;
		default:
		$MSG = "";
		break;
