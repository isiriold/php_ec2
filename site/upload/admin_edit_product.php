...
<form id="theForm" name="theForm" action="?TASK=<?php echo $CURTASK; ?>" method="post" class="ajax_form" enctype="multipart/form-data" onsubmit="return checkit('theForm');">
...
<div class="fl1 wid1"><label for="productname">Name:</label> </div>
...
<div class="fl1 wid1"><label for="productdesc">Description:</label>  </div>
...
<div id="r_productactive" class="lpad1">
	 <div class="fl1 wid1">	 <label for="productactive">Active:</label> </div>
	 <div class="fl1 padl1"> 
...
<div id="r_logo" style="vertical-align:top;" class="lpad1">
	 <div class="fl1 wid1"><label for="productlogo">Logo:</label></div>
	 <div class="fl1">
<?php echo <<<zz
...
<?php
echo makeButton( $type="BUTTON", $display="Save Product", $id="", $func="checkit('');", $class="fl1" );
if( is_numeric( $Lproductid) ) {
echo makeButton( $type="BUTTON", $display="New Detail", $id="", $func="newDetail('workerdiv','workerdivinner','workerdivcontent','?TASK=AJAXPRODDETAIL&NEWITEM=1&IDX=NEW&PRODUCTID=$Lproductid&RETURNTASK=ADMINPRODUCT','workerdivtitle');", $class="fl1" );
}
?>
...
foreach( $qTypes[0] as $deet ) { //checker
	...
	if( $tCode == "PDIMAGE" || $tCode == "PDMOVIE" || $tCode == "PDDLDOC" ) {
echo <<<zz
	 case "$tID":
		 FE = "block";
		 break;
